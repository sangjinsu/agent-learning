...
from agent_learning.example_support import print_learning_sections, split_verbose_flag
...
def main() -> None:
    positional, verbose = split_verbose_flag(sys.argv[1:])
    flow, chapter = parse_cli(positional)
...
def print_result(result: MCPDemoResult, *, verbose: bool) -> None:
    if verbose:
    ...
    if result.prompt_text:
        label = "prompt messages" if verbose else "prompt result"
        print(f"{label}: {result.prompt_text}")

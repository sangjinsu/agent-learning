...
from agent_learning.example_support import parse_example_args, print_learning_sections, print_mapping, print_messages
from agent_learning.llm.prompting import chat_input, format_messages


def main() -> None:
    args = parse_example_args(sys.argv[1:], "How does ChatPromptTemplate work?")
    ...
    print(f"final user message: {messages[-1].content}")
    if args.verbose:
